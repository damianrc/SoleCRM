...
import { validateBody, validateQuery, contactSchema, contactUpdateSchema, taskSchema, noteSchema, activitySchema, paginationSchema, contactFilterSchema, bulkContactsSchema, bulkDeleteSchema } from '../middleware/validation.js';
...
router.post('/', validateBody(contactSchema), async (req, res) => {
    try {
        const { name, phone, address, suburb, contactType, leadSource, status } = req.body;
        const userId = req.user.id;
