...
// Validation schema for updating custom field values
const updateCustomFieldsSchema = z.record(z.string(), z.any());

// Helper function to validate and parse field value based on type
const parseFieldValue = (value: any, fieldType: string): string => {
  switch (fieldType) {
    case 'NUMBER':
...
const parseStoredValue = (value: string, fieldType: string): any => {
  switch (fieldType) {
    case 'NUMBER':
...
router.put('/:contactId/custom-fields', async (req, res) => {
  try {
    ...
    const contactId = req.params.contactId;
    const validatedData = updateCustomFieldsSchema.parse(req.body);
    ...
    for (const property of customProperties) {
      if (property.isRequired && (!validatedData[property.fieldKey] || validatedData[property.fieldKey] === '')) {
    ...
    const updates = [];
    for (const [fieldKey, value] of Object.entries(validatedData)) {
